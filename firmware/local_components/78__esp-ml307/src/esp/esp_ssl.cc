...
bool EspSsl::Connect(const std::string& host, int port) {
    ...
    if (event_group_ == nullptr) {
        ...
        esp_tls_conn_destroy(tls_client_);
        tls_client_ = nullptr;
        ESP_LOGE(TAG, "Failed to create SSL receive event group");
        return false;
    }
    ...
    if (xTaskCreate([](void* arg) {
        ...
        ssl->receive_task_handle_ = xTaskGetCurrentTaskHandle();
        ssl->ReceiveTask();
...
void EspSsl::WaitForReceiveTaskExit() {
    ...
    // more after ReceiveTask() returns. Wait until that final access completes.

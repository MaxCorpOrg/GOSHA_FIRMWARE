# AGENTS.md

Эта папка отвечает за конкретную плату `gosha-v1`.

## Перед работой прочитать

1. `/home/max/GOSHA_FIRMWARE/AGENTS.md`
2. `/home/max/GOSHA_FIRMWARE/docs/PROJECT_STATUS_RU.md`
3. `/home/max/GOSHA_FIRMWARE/docs/HARDWARE_MANIFEST_RU.md`
4. `/home/max/GOSHA_FIRMWARE/docs/PIN_MAP_RU.md`

## Что здесь менять

- `config.h`
  - аппаратные и функциональные параметры платы
- `config.json`
  - описание платы для сборочного слоя
- `otto_robot.cc`
  - основная логика профиля платы
- `otto_emoji_display.*`
  - экран, эмоции, локальный интерфейс
- `websocket_control_server.*`
  - локальный управляющий `WebSocket`
- `oscillator.*`, `otto_movements.*`, `otto_controller.cc`
  - движения и поведение робота

## Главные правила

- Не менять выводы, питание, аудио и приводы без сверки с аппаратным манифестом и pin map.
- Если правка влияет на слово пробуждения, звук, экран или локальный портал, это обязательно фиксировать в документации.
- Цвет анимированных глаз `gosha-v1` задаётся параметром `DEFAULT_EMOJI_TINT` в `firmware/main/CMakeLists.txt` и применяется сборщиком ресурсов. Не править GIF-файлы внутри `managed_components/` вручную: это внешняя, неотслеживаемая зависимость.
- После изменения палитры глаз обязательно:
  - проверить все 21 GIF-эмоции с сохранением размеров, кадров и задержек;
  - выполнить каноническую сборку `python3 scripts/release.py gosha-v1 --name gosha-v1`;
  - при точечной установке записывать только совместимый `generated_assets.bin` в раздел `assets`;
  - помнить, что этот раздел содержит также модель и метаданные слова пробуждения, поэтому ресурс нельзя переносить на произвольную старую прошивку.
- После значимой правки обновлять:
  - `/home/max/GOSHA_FIRMWARE/docs/PROJECT_STATUS_RU.md`
  - `/home/max/GOSHA_FIRMWARE/docs/AGENT_CHECKPOINT_RU.md`
  - `/home/max/GOSHA_FIRMWARE/docs/HARDWARE_MANIFEST_RU.md`, если изменилась реальная аппаратная конфигурация
  - `/home/max/GOSHA_FIRMWARE/docs/PIN_MAP_RU.md`, если изменились или подтвердились выводы

## Язык и формат

- Пояснения по плате `gosha-v1`, слову пробуждения, аудио, экрану и локальному порталу пиши нормальным русским техническим языком.
- Не используй русско-английский суржик в обычном тексте.
- Если термин технический важен, сразу поясняй его смысл по-русски.
- Для типовых слов используй такие формы:
  - `verified online` — подтверждённо в сети
  - `detect` — обнаружение
  - `handshake` — проверочный обмен запрос-ответ
  - `scope` — область изменений
  - `branch` — ветка
  - `commit` — коммит
  - `push` — отправка изменений
  - `pull` — получение изменений
- Английский оставляй только для команд, путей, имён файлов, функций, переменных, веток, коммитов и кода.

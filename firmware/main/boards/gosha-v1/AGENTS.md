...
- Физический сервопривод левой руки отключён от платы и не должен подключаться
  обратно до ремонта. USB/serial и установка образа разрешены по
  `docs/HARDWARE_DEVELOPMENT_POLICY_RU.md`; команды движения, servo sequence,
  `Home` и `self.otto.set_trim` требуют отдельной явной команды владельца.

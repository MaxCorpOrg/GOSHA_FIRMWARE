## Motion Studio: сохранная библиотека — 24 сентября 2026

Ветка `codex/motion-studio-library-20260924` от принятой базы 23 сентября:
три видимых пакета, сохранное обновление через запасной слот, постоянный ID
и миграция прежних A/B с восстановлением после прерывания первой записи.
Host-тесты и сборка app-only прошли; робот не прошивался и движения не
запускались. Кандидат SHA-256 `8bcb0c3a…f1f70737a0b8`, подробности:
[MOTION_STUDIO_LIBRARY_2026-09-24_RU.md](MOTION_STUDIO_LIBRARY_2026-09-24_RU.md).

## Перенос на другое устройство — 19 сентября 2026

Основная точка продолжения — `codex/restore-robot-movements-20260914`:
27 обычных движений с принятыми скоростями/амплитудами, duplex/MCP,
Motion Studio, hardware runner, persistence и targeted delete сохранены.
Последний установленный app остаётся из `a63ac87`, SHA-256
`9ba5e327eb360c12ebf437a679dfe525b3c8064b77cd51dd99a7cd5cc5bfdfb4`.
Перенос не меняет код устройства и не является новой аппаратной проверкой.
Host-тесты теперь берут SDK из `IDF_PATH`, без пути к старому компьютеру:
перед запуском подключите окружение ESP-IDF 5.5.2 через его `export.sh`.
`managed_components`, build и локальный sdkconfig в Git не переносятся;
зависимости и конфигурацию собирают в новом окружении по инструкции профиля.
Общий маршрут: `AI_ROBOTS/docs/TRANSFER_TO_ANOTHER_DEVICE_2026-09-19_RU.md`
в ветке `codex/execute-project-plan-20260905`. Старые записи ниже — история.

## 14 сентября: восстановленные движения приняты владельцем

В ветке `codex/restore-robot-movements-20260914` подготовлен полный прежний
каталог с исходными скоростями/амплитудами по выбору владельца, отдельно от
ограничений Motion Studio. Host-проверки и app-only установка PASS.
Текущий app `9ba5e327…fdfb4` из `a63ac87`; владелец подтвердил, что движения заработали. `8df041e3…cdf55d` сохранён для отката.
Подробности: [LEGACY_MOVEMENTS_RESTORE_RU.md](LEGACY_MOVEMENTS_RESTORE_RU.md).

# AGENT CHECKPOINT

Исправленная диагностика v2 собрана и проверена: app `671bbbdd…`,
Firmware source `c473857` / build `50cdb60`, 3 649 248 байт. Кадры
watchdog в ELF 48/64 байта вместо 1232/640, стек задачи остаётся 3584.
Host-проверки и независимая проверка исходников/процедур PASS.
Образ НЕ установлен; все агенты завершены. На роботе остаётся
восстановленный `caf1c368…`, панель подключена, рука не инициализирована.
Следующий шаг: текущее подтверждение опоры перед новой установкой.
Подробности: `docs/MOTION_STUDIO_PWM_WATCHDOG_REPAIR_2026-09-08_RU.md`.

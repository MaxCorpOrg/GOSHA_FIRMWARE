...
- Уже подтверждён промежуточный эффект:
  ...
  - порог `36` проверен как следующий минимальный шаг, но отклонён после ложного пробуждения без произнесения имени с вероятностью `0.412557`; на устройстве снова прошит и подтверждён `37`
...
- Проверить локальный контур `ws://192.168.4.1:8080/ws`.
  Отдельно проверить `initialize`, `tools/list`, `self.otto.get_status` и `self.otto.stop`, а также убедиться, что ответ приходит только в исходное WebSocket-соединение и что старый отложенный ответ после повторного подключения не попадает в новую сессию.

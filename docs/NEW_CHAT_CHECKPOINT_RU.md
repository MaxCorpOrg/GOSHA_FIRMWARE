...
- Последняя прошивочная правка без прошивки устройства:
  - для локального `WSControl` профиля `gosha-v1` JSON-RPC ответы `MCP` теперь отправляются только в исходное WebSocket-соединение;
...
- Проверить локальный контур `ws://192.168.4.1:8080/ws`.
  Отдельно проверить `initialize`, `tools/list`, `self.otto.get_status` и `self.otto.stop`, а также убедиться, что ответ приходит только в исходное WebSocket-соединение.

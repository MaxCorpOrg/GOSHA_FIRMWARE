...
- Текущий реальный статус устройства:
  ...
  - следующие задачи — проверка локального управления, настройка сети и проверка аппаратных выводов.
...
- Настроить сеть на устройстве и затем проверить голос, OTA, MCP, `stop`, `status`, `battery` и одно безопасное действие после стабилизации аппаратного контура.

...
- Неисправная левая серва физически отключена. USB/serial, flash,
  перезагрузка, motion, `trim`, servo sequence и осознанная проверка `:8080`
  разрешены по аппаратной политике.
...
- В этой исторической точке аппаратные действия были приостановлены. С
  `2026-09-03` прошивка, перезагрузка, движения, `set_trim`, servo sequence
  и осознанная проверка `:8080` разрешены.
...
- Проблемный левый сервопривод теперь подтверждённо отключён от платы; прошивка,
  перезагрузка и движение остальных приводов разрешены.
...
- Неисправный сервопривод левой руки должен оставаться отключённым. Движения
  остальных приводов и `set_trim` разрешены по аппаратной политике.
...
- На новом устройстве держать неисправную левую серву отключённой; движения
  остальных приводов и функциональные проверки разрешены.

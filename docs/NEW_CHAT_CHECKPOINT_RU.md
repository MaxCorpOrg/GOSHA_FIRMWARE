...
- В ветке добавлен локальный read-only контракт `gosha.identity.get/result`, который возвращает Android аппаратный `device_id` по существующему `/ws`, не пересылая ответ в облако.
- Полная сборка `gosha-v1` прошла, но новый образ не прошивался.

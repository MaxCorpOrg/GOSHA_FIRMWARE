...
- Текущий реальный статус устройства:
  ...
  - следующие задачи — подтверждение русского пользовательского слоя, проверка локального управления, настройка сети, проверка OTA через `/gosha/ota/` и проверка аппаратных выводов.

## Что делать следующим

- Подтвердить аппаратный манифест и pin map по реальным журналам устройства.
- Подтвердить на экране и на слух русский пользовательский слой после новой прошивки.
- Проверить локальный контур `ws://192.168.4.1:8080/ws`.
- Настроить сеть на устройстве и сначала подтвердить, что OTA идёт на `/gosha/ota/` без `404`.

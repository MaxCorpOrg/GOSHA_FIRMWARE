...
## Самая свежая точка 2026-07-22

- Экранная ветка: `feature/firmware-orange-eyes` в `/home/max/worktrees/gosha/firmware-orange-eyes`, база `af4de9c`.
- Для `gosha-v1` подготовлен параметрический оранжевый цвет глаз `#FF8A00`; остальные платы не затронуты.
- Все 21 GIF-эмоции проверены: кадры, задержки и сжатые данные сохранены, меняются только палитры.
- Каноническая сборка прошла; SHA-256 `generated_assets.bin`: `871b1b74b98741c48c34d178588ae816fdba2e8197af1a36c46a3667acf70729`.
- ИИ-офис `task-20260722T131136Z-read-only-review-firmware-orange-eyes` не нашёл P0/P1/P2.
- Новый `generated_assets.bin` установлен на тестовый робот только в раздел `assets` по `0x800000`; аппаратная проверка контрольной суммы прошла, приложение и настройки не менялись.
- Перед установкой сохранена резервная копия `/tmp/gosha-assets-before-orange-20260722.bin`, SHA-256 `30ebc885107ea712d210112005d86410f49dbd8f4d095f589b4b1840dadd7f8a`.
- После записи UART подтвердил стабильную память и штатную связь без паники или повторной загрузки.
- Следующий шаг: визуально подтвердить оттенок на экране, затем проверить основные эмоции, слово пробуждения и один голосовой диалог.

...
- В журнале нет ожидаемых `wifi_prompt_diag`, `EnterWifiConfigMode` и `Brownout`; зафиксирована только интенсивная нагрузка `WSControl` от Android-клиента. Числовое сравнение двух запусков не получено, причина прежнего скачка громкости не доказана.
- Следующее аппаратное действие только при повторном проявлении дефекта:
  - сначала подтвердить активный OTA-раздел и версию образа по загрузочному журналу;
  - убрать `WebSocket`-нагрузку;
  - повторить два запуска с UART-съёмом.

...
- Отдельное рабочее дерево: `<FIRMWARE_ORANGE_EYES_WORKTREE>`.
...
- Перед установкой сняты точные резервные копии действовавшей пары: приложение `<PRIVATE_APP_BACKUP>`, SHA-256 `8046e3804b3d1f5f741e81c7e821894dffd618941cad6792742da4918ef31fd4`; ресурсы `<PRIVATE_ASSETS_BACKUP>`, SHA-256 `871b1b74b98741c48c34d178588ae816fdba2e8197af1a36c46a3667acf70729`.
...
- Обезличенное доказательство общего прогона: `<PRIVATE_VALIDATION_EVIDENCE>/task-20260723T111058Z-read-only/live-validation/acceptance-evidence.json`, SHA-256 `600c3d526dddf95a39c38c1cf126a952d02a2e6b7b506f8a591b6f94d7690f0e`.
...
- Создан отдельный локальный репозиторий `<FIRMWARE_WORKSPACE>`.
...
- Поднято локальное окружение сборки:
  - `ESP-IDF 5.5.2`
  - путь: `<ESP_IDF_ROOT>`
  - пользовательские инструменты сборки: `cmake`, `ninja` в `<USER_TOOL_BIN>`
...
- Локализован и исправлен новый дефект локального звука в режиме настройки `Wi‑Fi`:
  ...
  - исправленный `gosha.bin` уже собран через `ninja` и прошит на тестовое устройство через `<ROBOT_SERIAL_PORT>`

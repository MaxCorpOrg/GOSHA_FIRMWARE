...
## Текущая диагностическая работа 2026-07-10
...
- Ожидаемые `wifi_prompt_diag`, `EnterWifiConfigMode` и `Brownout` в журнал не попали; присутствует интенсивный поток `WSControl` от Android-клиента. Числовое сравнение двух запусков не получено, поэтому причина прежнего скачка громкости остаётся недоказанной.
- Общий ограничитель громкости `30` отклонён: на живой проверке он делал подсказку слишком тихой.
- При следующем воспроизведении дефекта сначала подтвердить активный OTA-раздел и версию образа по загрузочному журналу, затем повторить проверку без `WebSocket`-нагрузки.

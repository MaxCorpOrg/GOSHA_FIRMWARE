...
- Артефакты этой проверки подтверждают статическую сборку. Для установки
  ...
  USB/serial, flash, reboot, motion и trim по аппаратной политике.
...
- В этой исторической точке аппаратные действия ещё не выполнялись. С
  `2026-09-03` прошивка, перезагрузка, движения, `self.otto.set_trim`, servo
...
- Неисправный левый сервопривод теперь физически отключён от платы; прошивка,
  перезагрузка и команды остальным приводам разрешены.
...
- Левую серву необходимо держать отключённой и не подталкивать под питанием.
  Настройка `trim` и движения исправных приводов разрешены по аппаратной
  политике.
...
- На новом устройстве держать неисправную левую серву отключённой; движения
  остальных приводов разрешены по аппаратной политике.

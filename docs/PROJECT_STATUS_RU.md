...
- Реализован адресный путь ответов локального `MCP` для `gosha-v1` через `WSControl`:
  ...
  - в `firmware/main/boards/gosha-v1/websocket_control_server.*` локальный JSON-RPC ответ отправляется только в исходный файловый дескриптор WebSocket-соединения;
  - отправка идёт через штатную очередь `httpd_queue_work` и `httpd_ws_send_frame_async`, с проверкой активного WebSocket-дескриптора через `httpd_ws_get_fd_info`;
...
- Локальный управляющий контур `ws://192.168.4.1:8080/ws` теперь должен отвечать на JSON-RPC `MCP`-запросы только в то же WebSocket-соединение, из которого пришёл запрос.
...
- Подключиться к точке доступа устройства и допроверить локальный контур управления:
  ...
  - `self.otto.stop`
  При проверке убедиться, что ответ приходит только в исходное WebSocket-соединение, а не широковещательно всем клиентам.

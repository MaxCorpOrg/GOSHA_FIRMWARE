...
- Исправлен адрес OTA по умолчанию для собственной платформы `Гоша`:
  ...
  - новый адрес по умолчанию:
    - `http://151.241.228.232:18876/xiaozhi/ota/`
...
- Следующий фактический блокер уже не в дисплее и не в локальном `WebSocket`, а в следующем слое проверки:
  ...
  - нужно подтвердить уже на живой сети, что после перепрошивки устройство больше не получает `404` при обращении к OTA-контуру `Гоша`
  - устройство пока не переведено к полной проверке голоса, OTA и облачного контура, потому что сначала нужно проверить локальный контур, исправленный адрес OTA и аппаратные выводы

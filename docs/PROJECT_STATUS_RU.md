...
- Текущее рабочее решение по слову пробуждения:
  ...
  - порог `38` работал приемлемо, но оператор попросил совсем немного повысить чувствительность
  - текущая живая рабочая точка — `37`
  - порог `36` отдельно собран и прошит для минимального сравнительного шага; положительные обращения проходили, но в отрицательном прогоне робот ложно пробудился без произнесения имени с вероятностью `0.412557`, поэтому `36` отклонён
  - переход на `50` пока не нужен
- Новый образ с порогом `37` канонически собран через `scripts/release.py`, прошит на устройство и подтверждён по живому загрузочному журналу:
  - `set det threshold to 0.370000`
  - после отклонения `36` образ `37` повторно собран и прошит без очистки `NVS`; робот вернулся в домашнюю сеть с адресом `192.168.0.103` и загрузился из `ota_0`
  ...
  - это короткая приёмка; расширенный прогон с `20` обращениями и проверкой эха собственного динамика ещё нужен
...
- Реализован адресный путь ответов локального `MCP` для `gosha-v1` через `WSControl`:
  - `McpServer` получил перегрузки `ParseMessage` с `McpReplySender`, то есть с передаваемой функцией отправки ответа;
  - старые перегрузки `ParseMessage` оставлены и по умолчанию продолжают отправлять ответы через `Application::SendMcpMessage`, поэтому облачный `MCP`-путь не изменён;
  - отправитель ответа протянут через `ReplyResult`, `ReplyError`, `GetToolsList` и `DoToolCall`, включая отложенный вызов инструмента с захватом отправителя по значению;
  - в `firmware/main/boards/gosha-v1/websocket_control_server.*` локальный JSON-RPC ответ привязан к исходному файловому дескриптору и поколению WebSocket-сессии;
  ...
  - отправка идёт через штатную очередь `httpd_queue_work` и `httpd_ws_send_frame_async`, с проверкой активного поколения сессии и активного WebSocket-дескриптора через `httpd_ws_get_fd_info`;
...
- Базовый живой голосовой цикл уже подтверждён, поэтому следующая точка остановки — доводка качества и устойчивости:
  ...
  - на пороге `37` короткий живой тест прошёл успешно; остаётся расширенный акустический прогон с `20` обращениями и отрицательными фразами
...
- Локальный управляющий контур `ws://192.168.4.1:8080/ws` теперь должен отвечать на JSON-RPC `MCP`-запросы только в то же WebSocket-соединение, из которого пришёл запрос, и отбрасывать устаревший ответ после повторного подключения с тем же файловым дескриптором.
  Следующая проверка этого пункта должна быть живой: подключиться к точке доступа устройства штатным WebSocket-клиентом, без `nc` и без сырого TCP, и проверить `initialize`, `tools/list`, `self.otto.get_status` и `self.otto.stop`.
...
- На уже прошитом пороге `37` выполнить расширенную проверку: не меньше `20` обращений, обычная и громкая речь без имени, паузы после ответов собственного динамика.
- Отдельно оценить качество звука и при необходимости согласовать частоты между сервером и устройством.
- Если на `37` появятся устойчиво воспроизводимые ложные пробуждения, вернуть `38` и отдельно сравнить:
...
- Подключиться к точке доступа устройства и допроверить локальный контур управления:
  ...
  - `self.otto.stop`
  При проверке убедиться, что ответ приходит только в исходное WebSocket-соединение, а не широковещательно всем клиентам, и что старый отложенный ответ после повторного подключения не попадает в новую сессию.

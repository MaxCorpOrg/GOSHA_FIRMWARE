...
## Текущая работа: оранжевые глаза `gosha-v1`
...
- Для `gosha-v1` задан оранжевый оттенок глаз `#FF8A00` через параметр `DEFAULT_EMOJI_TINT`.
...
- Проверены все 21 GIF-эмоции: размеры, число кадров, задержки и сжатые данные анимации сохранены; меняются только байты палитр.
- Каноническая сборка `python3 scripts/release.py gosha-v1 --name gosha-v1` завершилась успешно.
- SHA-256 `firmware/build/generated_assets.bin`: `871b1b74b98741c48c34d178588ae816fdba2e8197af1a36c46a3667acf70729`.
- Раздел ресурсов занимает `5401743` байта и укладывается в выделенные `8 MB`.
- Независимая проверка ИИ-офиса `task-20260722T131136Z-read-only-review-firmware-orange-eyes` не нашла P0/P1/P2.
- Новый `generated_assets.bin` записан на тестовый робот только в раздел `assets` по адресу `0x800000`; приложение, NVS, таблица разделов и загрузчик не менялись.
- Перед записью сохранена полная резервная копия прежнего раздела: `/tmp/gosha-assets-before-orange-20260722.bin`, SHA-256 `30ebc885107ea712d210112005d86410f49dbd8f4d095f589b4b1840dadd7f8a`.
- Аппаратная проверка контрольной суммы записи прошла. Последующее наблюдение по UART подтвердило стабильную память, штатные HTTP-сеансы событий и проверочные обмены `WSControl` без паники или повторной загрузки.
- Цвет на физическом дисплее нужно подтвердить визуально оператору; автоматическая проверка подтверждает состав ресурса и его палитру, но не восприятие оттенка конкретной матрицей.

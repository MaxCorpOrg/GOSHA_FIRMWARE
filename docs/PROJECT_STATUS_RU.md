...
- Для безопасного различения физических роботов добавлен локальный read-only контракт `gosha.identity.get/result` на существующем WebSocket `/ws`.
...
- Каноническая полная сборка `gosha-v1` через `scripts/release.py` завершилась успешно; образ не записывался на устройство.
